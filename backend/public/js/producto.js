...
function renderProductDetails(p) {
  ...
  let badgeHtml = '';
  if (status === 'pending') badgeHtml = `<div style="margin-top:0.3rem;"><span class="badge badge-pending"><i class="fa-solid fa-hourglass-half"></i> ${i18next.t('catalog.pendingStatus')}</span></div>`;
  else if (status !== 'verified') badgeHtml = `<div style="margin-top:0.3rem;"><span class="badge badge-rejected"><i class="fa-solid fa-xmark"></i> ${i18next.t('catalog.rejectedStatus')}</span></div>`;
  ...
  container.innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:3rem;align-items:start;">
      ...
      <div>
        ...
        <div style="display:flex;gap:0.5rem;flex-wrap:wrap;margin-bottom:1.5rem;font-size:0.85rem;color:var(--color-muted);">
          ...
          ${p.materials ? `<span style="background:var(--color-bg2);padding:0.2rem 0.6rem;border-radius:999px;"><strong>${i18next.t('product.materialsLabel')}:</strong> ${p.materials}</span>` : ''}
          ${p.dimensions ? `<span style="background:var(--color-bg2);padding:0.2rem 0.6rem;border-radius:999px;"><strong>${i18next.t('product.dimensionsLabel')}:</strong> ${p.dimensions}</span>` : ''}
          ${p.weight ? `<span style="background:var(--color-bg2);padding:0.2rem 0.6rem;border-radius:999px;"><strong>${i18next.t('product.weightLabel')}:</strong> ${p.weight}</span>` : ''}
        </div>
        ...
        <!-- Cultural Story -->
        <h2 style="font-family:'Crimson Pro',serif;font-size:1.5rem;margin-bottom:1rem;">${i18next.t('product.historyHeading')}</h2>
        <div style="display:grid;gap:1rem;">
          <div style="background:var(--color-bg2);padding:1rem;border-radius:var(--radius);">
            <div style="font-weight:600;font-size:0.85rem;text-transform:uppercase;letter-spacing:0.05em;color:var(--color-muted);margin-bottom:0.4rem;"><i class="fa-solid fa-seedling"></i> ${i18next.t('product.originLabel')}</div>
            <div>${p.cultural_origin || `<em style="color:var(--color-muted);">${i18next.t('admin.notSpecified')}</em>`}</div>
          </div>
          <div style="background:var(--color-bg2);padding:1rem;border-radius:var(--radius);">
            <div style="font-weight:600;font-size:0.85rem;text-transform:uppercase;letter-spacing:0.05em;color:var(--color-muted);margin-bottom:0.4rem;"><i class="fa-solid fa-hammer"></i> ${i18next.t('product.techniqueLabel')}</div>
            <div>${p.technique || `<em style="color:var(--color-muted);">${i18next.t('admin.notSpecified')}</em>`}</div>
          </div>
          <div style="background:var(--color-bg2);padding:1rem;border-radius:var(--radius);">
            <div style="font-weight:600;font-size:0.85rem;text-transform:uppercase;letter-spacing:0.05em;color:var(--color-muted);margin-bottom:0.4rem;"><i class="fa-solid fa-wand-magic-sparkles"></i> ${i18next.t('product.significanceLabel')}</div>
            <div>${p.significance || `<em style="color:var(--color-muted);">${i18next.t('admin.notSpecified')}</em>`}</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <h2 style="font-family:'Crimson Pro',serif;font-size:1.5rem;margin-bottom:1rem;">${i18next.t('product.aboutArtisanHeading')}</h2>
    <div class="artisan-mini" style="max-width:500px; cursor:pointer;" onclick="window.location.href='/artesano.html?id=${p.artisan?.id}'">
      ...
      <div>
        <div style="font-weight:600; display:flex; align-items:center; gap:0.35rem;">
          ${p.artisan?.user?.full_name || `<i class="fa-solid fa-hammer"></i> ${i18next.t('catalog.anonymousArtisan')}`}
